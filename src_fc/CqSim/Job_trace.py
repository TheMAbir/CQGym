...
class Job_trace:
    ...
    def initial_import_job_file(self, job_file):
        ...
        self.reset_data()
        self.debug.line(4)
        self.i = 0      #用来指示 已经读取的作业的数量
        self.j = 0      #用来指示 读取的作业的行数
    ...
    def dyn_import_job_file(self):
        ...
        while (self.i<self.read_num or self.read_num<=0) and temp_n<self.read_input_freq:
            ...
            if self.i==self.read_num-1 or not tempStr :    # 当在文件中读取不到行数或已读取的行数到达上限时停止
                self.jobFile.close()
                return -1
                #break
            if (self.j>=self.anchor):
                ...
                tempInfo = {'id':int(temp_dataList[0]),\
                            ...
                            'autoAllocNode':int(temp_dataList[7]),\
                            'estRunTime':float(temp_dataList[8]),\
                            ...
                            'qosRunTime':int(temp_dataList[17]),\
                            'start':-1,\
                            'end':-1,\
                            'score':0,\
                            'state':0,\
    ...
    def job_submit (self, job_index, job_score = 0, job_est_start = -1):
        ...
        self.jobTrace[job_index]["state"]=1 #1代表作业作业提交
        self.jobTrace[job_index]["score"]=job_score
        self.jobTrace[job_index]["estStart"]=job_est_start #作业刚提交,预计开始运行时间为-1
        #将作业从提交列表中剔除,加入到等待列表中,且需要记录等待作业所需要的核数大小
        self.job_submit_list.remove(job_index)
        self.job_wait_list.append(job_index)
        self.job_wait_size += self.jobTrace[job_index]["autoAllocNode"]
        return 1
    
    def job_start (self, job_index, time):
        #self.debug.debug("* "+self.myInfo+" -- job_start",5)
        self.debug.debug(" "+"["+str(job_index)+"]"+" Req:"+str(self.jobTrace[job_index]['autoAllocNode'])+" Run:"+str(self.jobTrace[job_index]['run'])+" ",4)
