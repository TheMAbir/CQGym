...
import src_fc.Filter.Filter_job as filter_job

__metaclass__ = type
#   该类用于读取输入文件中的数据,将输入文件中的作业信息输出到../Fmt/train.csv中
class Filter_job_SWF(filter_job.Filter_job):
            ...
            if tempStr[0] != nr_sign:   # The job trace line
                if (temp_start>=self.anchor):
                    ...
                    for i in range(strNum):
                        ...
                        if (tempStr[i] == sept_sign):
                            ...
                            if k == 0:
                                ...
                                autoAllocNode = autoAllocNode + tempStr[i]
                            elif k == 8:
                                estRunTime = estRunTime + tempStr[i]
                            elif k == 9:
                                ...
                                qosRunTime = qosRunTime + tempStr[i]
                    ...
                    tempInfo = {'id':int(ID),\
                                ...
                                'autoAllocNode':int(autoAllocNode),\
                                'estRunTime':float(estRunTime),\
                                'reqMem':float(reqMem),\
                                'status':int(status),\
                                'userID':int(userID),\
                                'groupID':int(groupID),\
                    ...
                    if (self.input_check(tempInfo)>=0):
                        ...
                        f2.write(sep_sign)
                        f2.write(str(tempInfo['autoAllocNode']))
                        f2.write(sep_sign)
                        f2.write(str(tempInfo['estRunTime']))
                        f2.write(sep_sign)
                        ...
                        f2.write(sep_sign)
                        f2.write(str(tempInfo['qosRunTime']))
                        f2.write("\n")
    ...
    def input_check(self,jobInfo):
        if (int(jobInfo['run'])>int(jobInfo['estRunTime'])):
            jobInfo['run']=jobInfo['estRunTime']
        if (int(jobInfo['id'])<=0):
            return -2
        ...
        if (int(jobInfo['run'])<=0):
            return -4  
        if (int(jobInfo['estRunTime'])<=0):
            return -5
        if (int(jobInfo['autoAllocNode'])<=0):
            return -6
        return 1
    ...
    def output_job_data(self):
        ...
        for jobResult_o in self.jobList:
            ...
            f2.write(sep_sign)
            f2.write(str(jobResult_o['autoAllocNode']))
            f2.write(sep_sign)
            f2.write(str(jobResult_o['estRunTime']))
            f2.write(sep_sign)
            ...
            f2.write(sep_sign)
            f2.write(str(jobResult_o['qosRunTime']))
            f2.write("\n")
        f2.close()
